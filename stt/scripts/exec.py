"""Run elevenlabs-stt commands with the right Python environment."""
...
def _resolve_python(plugin_root: Path) -> list[str] | None:
    override = os.environ.get("ELEVENLABS_STT_PYTHON")
    if override:
        if Path(override).exists():
            return [override]
        print(
            f"Error: ELEVENLABS_STT_PYTHON not found: {override}",
            file=sys.stderr,
        )
        return None
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if python_prefix is None:
        if os.environ.get("ELEVENLABS_STT_PYTHON"):
            return 2
        print(
            "Warning: no managed environment found; falling back to system Python. "
            "Run /elevenlabs-stt:setup for a supported install.",
            file=sys.stderr,
        )
