...
class HotkeyListener:
    ...
    def _ensure_worker(self) -> None:
        """Ensure callback worker thread is running."""
        if self._worker_thread and self._worker_thread.is_alive():
            return
        self._worker_stop.clear()
        self._worker_thread = threading.Thread(
            target=self._event_worker,
            name="elevenlabs-stt-hotkey-worker",
            daemon=True,
        )
        self._worker_thread.start()

    def _event_worker(self) -> None:
        """Worker thread to process hotkey events."""
        while not self._worker_stop.is_set():
            ...
            try:
                ...
                logger.exception("Hotkey callback failed: %s", label)
    ...
    def _enqueue_event(self, label: str, callback: Optional[Callable[[], None]]) -> None:
        ...
        try:
            ...
            logger.warning("Dropping hotkey event '%s'; queue full", label)
    ...
    def start(self) -> bool:
        ...
        """
        if not _PYNPUT_AVAILABLE:
            logger.error("pynput unavailable; cannot start hotkey listener")
            return False
        ...
        try:
            ...
            if not self._listener.is_alive():
                logger.error("Hotkey listener failed to start")
                self.stop()
                return False
            return True
